...
**Payment Rails Architecture (Experimental):**
A pluggable payment system (`client/src/lib/rails/`) enables multiple payment methods. Currently implements PublicRail (EIP-3009 gasless transfers) and PrivateRail (stub for future Kohaku/Railgun integration). RailContext provides `getRailForPayment()` with automatic fallback from private to public. Privacy Mode toggle exists in Settings (disabled) as infrastructure for future private transfers. Rails are for payment logic only; balance aggregation remains in API layer.
