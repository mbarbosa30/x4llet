...
## Recent Changes
- **2025-12-16**: Fuzzy face matching for duplicate detection - Enhanced face verification to use cosine similarity (threshold 0.88) on raw 478-dimension embeddings in addition to exact hash matching. This catches the same person attempting to verify with different wallets even under varying conditions (lighting, angle, expressions). Schema updated with `embedding` (JSON) and `similarityScore` columns. Original verified records remain untouched; duplicate attempts create new records with status 'duplicate'. Improved Face Check banner UI with gradient background and clearer CTAs.
