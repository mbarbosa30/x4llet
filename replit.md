...
- **Fixed balance chart not updating after transfers**: Corrected TanStack Query cache invalidation to include full query keys `['/api/balance-history', address, chainId]` instead of partial keys. Updated AppHeader.tsx refresh handler, Receive.tsx payment success callback, and Send.tsx transaction success callback to properly invalidate balance history queries. The balance line chart now refreshes immediately when receiving transfers, sending payments, or manually refreshing.
- **Migrated to fawazahmed0 Currency API**: Replaced Frankfurter API with fawazahmed0 Currency API for exchange rates, adding support for previously unavailable currencies (ARS, NGN, KES, INR, CAD, AUD). Implemented dual CDN fallback (jsdelivr + currency-api.pages.dev) for reliability. Expanded currency support from 7 to 12 currencies total. Historical backfill now fetches 91 days of data sequentially with 100ms delay. Inflation calculation now uses 90 days instead of 30 for better accuracy.
