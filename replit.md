...
## Recent Changes
- **2025-12-16**: Face verification race condition fix - Added requestStartTime filter to prevent self-race conditions where a fresh verification after DB cleanup could match against records created during the same request. Admin delete buttons now properly show authentication errors instead of silently failing.
