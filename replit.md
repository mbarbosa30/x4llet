...
- **Bottom navigation**: Implemented fixed bottom navigation bar for mobile-first navigation with Signal (left), Wallet (middle), and Settings (right). Includes safe-area-inset-bottom padding for modern devices with notches, active state highlighting, and touch-optimized spacing. Simplified page headers by removing redundant back buttons and navigation elements.
