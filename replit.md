...
## Recent Changes
- **2025-12-16**: Face verification normalization fix - Embeddings are now normalized to unit vectors before storage AND before comparison, fixing false positive matches (0.998 similarity scores). Both incoming and stored embeddings are normalized for consistent cosine similarity. Face Check section now visible for all users including GoodDollar verified.
