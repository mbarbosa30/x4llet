...
## Recent Changes
- **2025-12-16**: Public Sybil API - Added `/api/public/v1/flagged-wallets` endpoint for MaxFlow integration. Returns all flagged wallets with scores, signals, and match counts. Features optional API key auth (SYBIL_API_KEY env var), rate limiting (60 req/min), and 5-minute caching. Threshold set to ≥4 points (requires 2 strong signals or 1 strong + 2 medium).
