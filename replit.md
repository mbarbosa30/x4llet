...
## Recent Changes
- **2025-12-16**: MaxFlow page restructured with tabbed interface - Trust tab (Face Check + MaxFlow signal + Vouch) and Claim tab (XP balance/claim, USDC redemption, SENADOR exchange, GoodDollar link). Tab state persists via localStorage, with URL query param support (`?tab=trust` or `?tab=claim`). Face Check card also added to Home page onboarding section for new users with 0 balance.
- **2025-12-16**: Face Check liveness verification - Added client-side face verification using MediaPipe Face Landmarker (CDN-loaded). Collapsible card in MaxFlow page with blink and head turn challenges. Face embeddings hashed (SHA-256) for privacy and stored in `face_verifications` table. Awards 50 XP on successful verification. Duplicate face detection feeds into sybil scoring (+3 points for same face on different devices). API endpoints: `GET /api/face-verification/:address`, `POST /api/face-verification/submit`.
